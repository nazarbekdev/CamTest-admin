...
class GenerateTestDefaultKey(GenericAPIView):
    ...
    def post(self, request):
        ...
        # Majburiy fanlar
        maj_ona_tili = SubjectTest.objects.filter(language_id=serializer.data['language'], subject_id=3).order_by('?')[:10]
        ...
        keys = {1: 'C', 2: 'C', 3: 'A', 4: 'C', 5: 'A', 6: 'B', 7: 'D', 8: 'B', 9: 'B', 10: 'D', 11: 'D', 12: 'B',
                ...
                35: 'A', 36: 'C', 37: 'A', 38: 'A', 39: 'B', 40: 'B', 41: 'D', 42: 'C', 43: 'B', 44: 'A', 45: 'A',
                46: 'B', 47: 'D', 48: 'D', 49: 'D', 50: 'A', 51: 'A', 52: 'C', 53: 'C', 54: 'C', 55: 'A', 56: 'B',
                57: 'A', 58: 'D', 59: 'A', 60: 'B', 61: 'D', 62: 'A', 63: 'B', 64: 'B', 65: 'A', 66: 'C', 67: 'A',
                68: 'A', 69: 'B', 70: 'B', 71: 'D', 72: 'C', 73: 'B', 74: 'A', 75: 'A', 76: 'B', 77: 'D', 78: 'D',
                ...
                for subject_name, tests, numbering_range in zip(subject_list, subjects, numbering_ranges):
                    ...
                    for i, test in enumerate(tests):
                        ...
                        if test.answers != '?':
                            ...
                            key = numbering_range[i]
                            if keys[key] == 'A':
                                if len(answer) <= 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answer} B) {answers_set[0]} C) {answers_set[1]} D) {answers_set[2]}", custom_style))
                                elif len(answer) > 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answer}  C) {answers_set[1]}", custom_style))
                                    elements.append(Paragraph(f"B) {answers_set[0]}  D) {answers_set[2]}", custom_style))
                                    # elements.append(Paragraph(f"", custom_style))
                                    # elements.append(Paragraph(f"", custom_style))
                            elif keys[key] == 'B':
                                if len(answer) <= 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answers_set[0]} B) {answer} C) {answers_set[1]} D) {answers_set[2]}", custom_style))
                                elif len(answer) > 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answers_set[0]}  C) {answers_set[1]}", custom_style))
                                ...
                                if len(answer) <= 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answers_set[0]} B) {answers_set[1]} C) {answer} D) {answers_set[2]}", custom_style))
                                elif len(answer) > 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answers_set[0]}  C) {answer} ", custom_style))
                                    elements.append(Paragraph(f"B) {answers_set[1]}  D) {answers_set[2]}", custom_style))
                                    # elements.append(Paragraph(f"", custom_style))
                                    # elements.append(Paragraph(f"", custom_style))
                            elif keys[key] == 'D':
                                if len(answer) <= 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answers_set[0]} B) {answers_set[1]} C) {answers_set[2]} D) {answer}", custom_style))
                                elif len(answer) > 30:
                                    elements.append(Spacer(1, 2))
                                    elements.append(Paragraph(f"A) {answers_set[0]}  C) {answers_set[2]}", custom_style))
